...
mod rkyv {
    ...
    impl Archive for SetU32 {
        type Archived = ArchivedVec<rkyv::Archived<u32>>;
    ...
    impl<S: rancor::Fallible + Allocator + Writer + ?Sized> Serialize<S> for SetU32 {
        fn serialize(
            &self,
            serializer: &mut S,
    ...
    impl<D: rancor::Fallible + ?Sized> Deserialize<SetU32, D>
        for ArchivedVec<rkyv::Archived<u32>>
    where
        <D as rancor::Fallible>::Error: rancor::Source,
    {
        fn deserialize(
            &self,
            deserializer: &mut D,
        ) -> Result<SetU32, <D as rancor::Fallible>::Error> {
            let mut set = SetU32::new();
    ...
    fn roundtrip() {
        ...
        let deserialized = rkyv::from_bytes::<SetU32, rancor::Error>(&bytes).unwrap();
        ...
        let set = SetU32::from_iter(0..10000u32);
        let bytes = rkyv::to_bytes::<rancor::Error>(&set).unwrap();
        let deserialized = rkyv::from_bytes::<SetU32, rancor::Error>(&bytes).unwrap();
        assert_eq!(set, deserialized);
    }
